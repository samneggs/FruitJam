...
class DVI_RP2_HSTX():
    ...
    def begin(self, buffer, color_mode, height=None, width=None, bytes_per_pixel=None):
...
#         self._height, self._width, self._bytes_per_pixel = self._buffer.shape
        ...
        # Frame sync flag. Set by a tiny DMA marker block once per frame.
        self._frame_flag = array.array('I', [0])
        self._frame_one = array.array('I', [1])
    ...
    def _configure_dmas(self):
        ...
        if self._buffer_is_in_psram:
            ...
            # DMA streams 32-bit words into the row buffer, so require a word-sized row.
            if self._bytes_per_row & 3:
                raise ValueError("row buffer size must be a multiple of 4 bytes")

            self._row_buffer = array.array('I', [0] * (self._bytes_per_row >> 2))
    ...
    def _create_dma_ctrl_registers(self):
        ...
        # Executer control register for writing the per-frame sync flag.
        # It chains back to the dispatcher so the normal restart block follows.
        self._dma_ctrl_frame_marker = self._dma_executer.pack_ctrl(
            inc_read    = False,
            inc_write   = False,
            chain_to    = self._dma_dispatcher.channel,
            bswap       = False,
    ...
    def _create_control_blocks(self):
        ...
        num_cb += 1 # Frame marker control block
        ...
        # Control block for setting a per-frame sync flag in SRAM. This gives
        # application code a frame boundary marker without a Python DMA IRQ.
        self._cb_frame_marker = array.array('I', [
            addressof(self._frame_one), # READ_ADDR
            addressof(self._frame_flag), # WRITE_ADDR
            1, # TRANS_COUNT
            self._dma_ctrl_frame_marker, # CTRL_TRIG
        ])
        ...
        if self._buffer_is_in_psram:
            ...
            self._cb_xip_stream_start_nested = array.array('I', [
                ...
                self._height * self._width * self._bytes_per_pixel,
            ])
    ...
    def _assemble_control_blocks(self):
        ...
        # Mark the frame boundary, then restart the frame by reconfiguring
        # the control block dispatcher DMA.
        self._add_control_block(self._cb_frame_marker)
    ...
    def frame_ready(self):
        """
        Returns True if a frame boundary has occurred since the flag was cleared.
        """
        return self._frame_flag[0] != 0

    def clear_frame_ready(self):
        """
        Clears the frame boundary flag.
        """
        self._frame_flag[0] = 0

    def wait_frame(self):
        """
        Waits until the next frame boundary.
        """
        self._frame_flag[0] = 0
        while self._frame_flag[0] == 0:
            pass

    def deinit(self):
        """
        Stop DVI output and release driver resources.
        """
        # Stop the DMA chain from restarting at the next frame.
        try:
            self._stop()
        except Exception:
            pass

        # Abort owned DMA channels.
        mask = 0
        for name in ("_dma_dispatcher", "_dma_executer", "_dma_streamer"):
        ...
        if mask:
            machine.mem32[0x50000000 + 0x464] = mask   # DMA CHAN_ABORT
        ...
        # Return HSTX pins to GPIO inputs.
        for pin in (
            self._pin_clk_p, self._pin_clk_n,
            self._pin_d0_p, self._pin_d0_n,
            self._pin_d1_p, self._pin_d1_n,
            self._pin_d2_p, self._pin_d2_n,
        ):
            try:
        ...
        for name in (
            "_control_blocks", "_row_buffer",
            "_hstx_line_porch", "_hstx_line_vsync", "_hstx_line_active",
            "_frame_flag", "_frame_one", "_cb_frame_marker",
            "_buffer", "_hstx",
            "_dma_dispatcher", "_dma_executer", "_dma_streamer",
        ):
...
if __name__ == '__main__':
    ...
    SCREEN_W  = const(320)
    SCREEN_H  = const(240)   
    fb      = bytearray(SCREEN_W * SCREEN_H * 2)            # 2 bytes/pixel RGB565
    ...
    SCREEN_FB  = framebuf.FrameBuffer(fb, SCREEN_W, SCREEN_H, framebuf.RGB565)
    SCREEN_FB.rect(10,10,100,100,0xff,1)   
